...
int main() {
    ...
    const sf::Font font("/System/Library/Fonts/Supplemental/Arial Rounded Bold.ttf");
    ...
    while (window.isOpen()) {
        while (const std::optional event = window.pollEvent()) {
            ...
            if(const auto* mouseButtonPressed = event->getIf<sf::Event::MouseButtonPressed>()){
                if(mouseButtonPressed->button == sf::Mouse::Button::Left){
                    if(Run.mouseon(window) && Run.activated) {
                        ...
                        body.push_back(std::unique_ptr<Body>(new Body(0, 0, 7, dis(rd), dis(rd), dis(rd), font)));
            ...
            if(AddAct){
                if(const auto* keyPressed = event->getIf<sf::Event::KeyPressed>()){
                    ...
                    //소수점
                    if(keyPressed->scancode == sf::Keyboard::Scan::Period){
                        if(SetxAct) X.addvalue(".");
                        if(SetyAct) Y.addvalue(".");
                        if(SetvxAct) VX.addvalue(".");
                        if(SetvyAct) VY.addvalue(".");
        ...
        for(int i = 0; i < body_N; i++){
            ...
            if(draw_run == true){
                ...
                if(!paused) window.draw(&body[i]->vertices[0], body[i]->vertices.size(), sf::PrimitiveType::Points);
            }
            //draw가 false일 때는 merged가 만들어지지 않은 때이므로 예외처리
                ...
                window.draw(&body[i]->velocity[0], body[i]->velocity.size(), sf::PrimitiveType::Lines);
